...
void MainWindow::slot_about() {
  ...
  QMessageBox::about(
      centralWidget, "关于",
      "《扫雷》是一款大众类的益智小游戏，于1992年发行。游戏目标是在最短的时间内"
      "根据点击格子出现的数字找出所有非雷格子，同时避免踩雷，踩到一个雷即失败。"
      "\n"
      "  游戏的基本操作包括左键单击（Left Click）、右键单击（Right "
      "Click）两种。其中左键用于打开安全的格子，推进游戏进度，右键用于标记地雷"
      "，以辅助判断，或为接下来的双击做准备；双击在一个数字周围的地雷标记完时，"
      "相当于对数字周围未打开的方块均进行一次左键单击操作：\n"
      "  "
      "左键单击：在判断出不是雷的方块上按下左键，可以打开该方块。如果方块上出现"
      "数字，则该数字表示其周围3×3区域中的地雷数（一般为8个格子，对于边块为5个"
      "格子，对于角块为3个格子。所以扫雷中最大的数字为8）；如果方块上为空（相当"
      "于0），则可以递归地打开与空相邻的方块；如果不幸触雷，则游戏结束。\n"
      "  "
      "右键单击：在判断为地雷的方块上按下右键，可以标记地雷（显示为小红旗）。重"
      "复一次或两次操作可取消标记（如果在游戏菜单中勾选了“标记(?)"
      "”，则需要两次操作来取消标雷）。\n");
  m_timer->start(1000);
}
